...
func (a *UseCase) PutEvent(ctx context.Context, in *dto.PutEventIn) (*dto.GetGroupsOut, error) {
	...
	log.Info("getting collaborator ids")

	ids, err := a.storage.GetCollaboratorIds(ctx, in.Collaborators)
	if err != nil {
		log.Error("failed to get collaborator ids", err.Error(), slog.String("event", in.EventId))
		return nil, err
	}

	log.Info("updating collaborator list")

	err = a.storage.UpdateEventCollaborators(ctx, in.EventId, ids)
	if err != nil {
