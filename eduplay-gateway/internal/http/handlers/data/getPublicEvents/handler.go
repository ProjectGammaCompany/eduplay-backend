...
type UseCase interface {
	GetPublicEvents(ctx context.Context, filters *dataModel.EventBaseFilters) (*dataModel.GetPublicEventsOut, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.data.getPublicEvents"
		...
		filters := &dataModel.EventBaseFilters{}
