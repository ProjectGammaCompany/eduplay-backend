...
type UseCase interface {
	GetProfile(ctx context.Context, userId string) (*userModel.Profile, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.users.getProfile"
		...
		profile, err := uc.GetProfile(request.Context(), accessClaims.ID)
		if err != nil {
		...
		render.JSON(writer, request, profile)
	}
}
