...
func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.users.signOutUser"
		...
		if err != nil {
			if errors.Is(err, storage.ErrInvalidAccessToken) {
				...
				render.JSON(writer, request, storage.ErrInvalidAccessToken.Error())
				return
			}
			if errors.Is(err, storage.ErrAccessTokenExpired) {
				log.Error("access token expired", slog.String("error", err.Error()))
				writer.WriteHeader(http.StatusUnauthorized)
				render.JSON(writer, request, storage.ErrAccessTokenExpired.Error())
				return
			}
		...
		err = uc.SignOutUser(context.Background(), accessToken)
		if err != nil {
			log.Error("failed to sign out user")
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, lib.Error("failed to sign out user"))
			return
		}

		log.Info("successfully signed out user")
