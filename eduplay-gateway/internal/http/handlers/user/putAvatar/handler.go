...
type UseCase interface {
	PutAvatar(ctx context.Context, pd *model.Profile) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.users.putAvatar"
		...
		message, err := uc.PutAvatar(context.Background(), req)
		if err != nil {
			log.Error("failed to set new avatar")
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, lib.Error("failed to set new avatar"))
			return
		}

		log.Info("success to set new avatar", slog.Any("response", message))
