...
type UseCase interface {
	GetUserData(ctx context.Context, token string) (*model.UserInfo, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.users.getUserData"
		...
		userInfo, err := uc.GetUserData(context.Background(), accessToken)
		if err != nil {
			log.Error("failed to get user info")
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, lib.Error("failed to get user info"))
			return
		}

		log.Info("success to get user info", slog.Any("response", userInfo))
