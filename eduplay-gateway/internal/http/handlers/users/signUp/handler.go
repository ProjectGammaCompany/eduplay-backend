...
type UseCase interface {
	SignUp(ctx context.Context, pd *model.SignUpIn) (*model.Credentials, error)
}
...
func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		credentials, err := uc.SignUp(context.Background(), pdModel)
		if err != nil {
			if errors.Is(err, storage.ErrUserAlreadyExists) {
				writer.WriteHeader(http.StatusConflict)
				render.JSON(writer, request, lib.Error("user already exists"))
				return
			}
