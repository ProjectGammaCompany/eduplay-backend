...
type UseCase interface {
	GetOwnedEvents(ctx context.Context, filters *eventModel.EventBaseFilters) (*eventModel.GetPublicEventsOut, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getOwnedEvents"
		...
		events, err := uc.GetOwnedEvents(request.Context(), filters)
		if err != nil {
			log.Error("failed to get user owned events", slog.String("error", err.Error()))
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, lib.Error("failed to get user owned events"))
			return
		}
