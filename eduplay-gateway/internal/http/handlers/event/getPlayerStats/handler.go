...
type UseCase interface {
	GetPlayerStats(ctx context.Context, in *eventModel.UserEventIds) (*eventModel.PlayerStats, error)
...
func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getPlayerStats"
		...
		var req eventModel.UserEventIds

		req.UserId = accessClaims.ID
		req.EventId = eventId

		playerStats, err := uc.GetPlayerStats(request.Context(), &req)
		...
		render.JSON(writer, request, playerStats)
	}
}
