...
type UseCase interface {
	PostEvent(ctx context.Context, pd *eventModel.PostEventIn) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.postEvent"
		...
		var req eventModel.PostEventIn
		req.OwnerId = accessClaims.ID
		...
		id, err := uc.PostEvent(request.Context(), &req)
