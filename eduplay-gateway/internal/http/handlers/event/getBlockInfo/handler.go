...
type UseCase interface {
	...
	GetBlockInfo(ctx context.Context, pd *eventModel.Id) (*eventModel.PostEventBlockIn, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getBlockInfo"
		...
		ret, err := uc.GetBlockInfo(request.Context(), &req)
