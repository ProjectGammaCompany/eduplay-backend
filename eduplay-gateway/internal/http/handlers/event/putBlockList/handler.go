...
type UseCase interface {
	...
	PutBlockList(ctx context.Context, pd *eventModel.PutBlockListIn) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.putBlockList"
		...
		var req eventModel.PutBlockListIn
		...
		if len(req.Blocks) == 0 {
			log.Error("no blocks provided")
			writer.WriteHeader(http.StatusBadRequest)
			render.JSON(writer, request, lib.Error("no blocks provided"))
			return
		}
		...
		ret, err := uc.PutBlockList(request.Context(), &req)
		if err != nil {
			log.Error(err.Error(), slog.String("error", err.Error()))
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, err)
