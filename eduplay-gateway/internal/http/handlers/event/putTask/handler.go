...
type UseCase interface {
	...
	PutTask(ctx context.Context, pd *eventModel.Task) (*eventModel.PutTaskOut, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.putTask"
		...
		if taskId == "" {
			...
			writer.WriteHeader(http.StatusBadRequest)
			render.JSON(writer, request, lib.Error("no task Id provided"))
			return
		}
		...
		if !isUUID {
			...
			writer.WriteHeader(http.StatusBadRequest)
			render.JSON(writer, request, lib.Error("invalid task id provided"))
			return
		}
