...
type UseCase interface {
	...
	DeleteBlockById(ctx context.Context, pd *eventModel.Id) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.deleteBlockById"
		...
		ret, err := uc.DeleteBlockById(request.Context(), &req)
