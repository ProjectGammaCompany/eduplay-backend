package postDownloadUserEventStatus
...
type UseCase interface {
	GetDownloadUserEventStatus(ctx context.Context, eventIds *eventModel.EventIds, userId string) (*eventModel.EventStatuses, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getDownloadUserEventStatus"
		...
		if len(req.EventIds) == 0 {
			log.Error(storage.ErrInvalidRequest.Error(), slog.String("error", storage.ErrInvalidRequest.Error()))
			writer.WriteHeader(http.StatusBadRequest)
			render.JSON(writer, request, storage.ErrInvalidRequest)
			return
		}

		ret, err := uc.GetDownloadUserEventStatus(request.Context(), &req, accessClaims.ID)
