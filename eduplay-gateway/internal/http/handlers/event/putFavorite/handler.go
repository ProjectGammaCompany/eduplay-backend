...
type UseCase interface {
	PutFavorite(ctx context.Context, filters *eventModel.PutFavorite) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.putFavorite"
		...
		var req *eventModel.PutFavorite
		...
		events, err := uc.PutFavorite(request.Context(), req)
		if err != nil {
			log.Error("failed to put favorite", slog.String("error", err.Error()))
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, lib.Error("failed to put favorite"))
			return
		}
