...
type UseCase interface {
	GetHistory(ctx context.Context, filters *eventModel.EventBaseFilters) (*eventModel.GetPublicEventsOut, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getHistory"
		...
		events, err := uc.GetHistory(request.Context(), filters)
		if err != nil {
