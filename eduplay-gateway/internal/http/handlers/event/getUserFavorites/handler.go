...
type UseCase interface {
	GetUserFavorites(ctx context.Context, filters *eventModel.EventBaseFilters) (*eventModel.GetPublicEventsOut, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getUserFavorites"
		...
		events, err := uc.GetUserFavorites(request.Context(), filters)
		if err != nil {
			log.Error("failed to get user favorites", slog.String("error", err.Error()))
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, lib.Error("failed to get user favorites"))
			return
		}
