...
type UseCase interface {
	PutTimestamp(ctx context.Context, in *eventModel.PutTimestampIn) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.putTimestamp"
		...
		var req eventModel.PutTimestampIn
		...
		message, err := uc.PutTimestamp(request.Context(), &req)
