...
type UseCase interface {
	PutNextStage(ctx context.Context, in *eventModel.EventBlockTaskUserIds) (string, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.putNextStage"
		...
		var req eventModel.EventBlockTaskUserIds
		...
		message, err := uc.PutNextStage(request.Context(), &req)
