...
type UseCase interface {
	...
	GetEventBlocks(ctx context.Context, eventId *eventModel.Id) (*eventModel.GetEventBlocksOut, error)
	GetEvent(ctx context.Context, eventId *eventModel.Id) (*eventModel.PostEventIn, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getEventBlocks"
		...
		blocks, err := uc.GetEventBlocks(request.Context(), &eventModel.Id{Id: id})
		if err != nil {
			log.Error(err.Error(), slog.String("error", err.Error()))
			writer.WriteHeader(http.StatusInternalServerError)
			render.JSON(writer, request, err)
