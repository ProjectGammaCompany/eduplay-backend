...
type UseCase interface {
	...
	GetBlockTasks(ctx context.Context, pd *eventModel.Id) (*eventModel.BlockTasksList, error)
}

func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		const op = "handlers.event.getBlockTasks"
		...
		ret, err := uc.GetBlockTasks(request.Context(), &req)
