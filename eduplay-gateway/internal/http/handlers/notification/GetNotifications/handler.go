...
type UseCase interface {
	GetNotifications(ctx context.Context, in *model.NotificationFilter) (*model.Notifications, error)
}
...
func New(log *slog.Logger, uc UseCase) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		notifs, err := uc.GetNotifications(request.Context(), filters)
		if err != nil {
		...
		render.JSON(writer, request, notifs)
	}
}
