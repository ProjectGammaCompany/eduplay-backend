...
import (
	...
	"eduplay-gateway/internal/http/handlers/event/getEditorUserStats"
	...
	"eduplay-gateway/internal/http/handlers/event/getJoinRequiredFields"
...
func EventRouter(router chi.Router, log *slog.Logger, cfg *config.Config) chi.Router {
	...
	router.Route("/event", func(r chi.Router) {
		...
		r.Get("/{eventId}/editorStats/{userId}", getEditorUserStats.New(log, events.New(log, eventClient, userClient)))
	})
